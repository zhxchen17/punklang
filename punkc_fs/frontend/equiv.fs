...
let rec natural_kind ctx c =
  ...
  | Cint -> Ktype
  | Capp (c0, c1) ->
    begin match natural_kind ctx c0 with
...
let rec equiv ctx c0 c1 k =
  ...
  | Kpi (k0, k1) ->
    equiv
      (Env.extend_kind ctx k0)
      (Capp (Subst.lift_con 1 c0, Cvar 0))
      (Capp (Subst.lift_con 1 c1, Cvar 0))
      k1
  | Kunit -> ()
...
and equiv_path ctx c0 c1 =
  ...
  | (Cvar i0, Cvar i1) ->
    if i0 = i1 then
      Env.lookup_kind ctx i0
    else
      raise (TypeError "equiv_path")
  | (Capp (c0a, c0b), Capp (c1a, c1b)) ->
    begin match equiv_path ctx c0a c1a with
      | Kpi (k0, k1) ->
  ...
  | _ -> raise (EquivTypeError ("equiv_path",
                                string c0,
                                string c1))
...
let rec selfify c k =
  ...
  | Ksing _ -> k
  | Kpi (k0, k1) ->
    Kpi (k1, selfify (Capp (Subst.lift_con 1 c, Cvar 0)) k1)
  | Kunit -> k
