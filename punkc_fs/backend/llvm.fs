...
let function_type tr tp =
    LLVM.FunctionType(tr, tp, false)

let struct_type ctx ts =
    LLVM.StructTypeInContext(ctx, ts, false)

let named_struct_type ctx name =
    LLVM.StructCreateNamed(ctx, name)

let var_arg_function_type tr tp =
    LLVM.FunctionType(tr, tp, true)

let struct_set_body t b packed =
    LLVM.StructSetBody(t, b, packed)

let declare_function name t mdl =
    let f = LLVM.GetNamedFunction(mdl, name) in
    if f.Pointer.Equals(0n) then
        LLVM.AddFunction(mdl, name, t)
    else
        f
...
let define_global name g mdl =
    let gv = LLVM.AddGlobal(mdl, LLVM.TypeOf(g), name) in
    LLVM.SetInitializer(gv, g);
    gv

let append_block ctx name func =
    LLVM.AppendBasicBlockInContext(ctx, func, name)
...
let build_array_alloca t v s builder =
    LLVM.BuildArrayAlloca(builder, t, v, s)
...
let build_cond_br p b0 b1 builder =
    LLVM.BuildCondBr(builder, p, b0, b1)
