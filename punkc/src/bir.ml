...
let type_by_name name { bir_type_decls } =
  Utils.find !bir_type_decls (fun (s, _) -> s = name)

let named_struct_type ctx name ({ bir_type_decls } as mdl) =
  match type_by_name name mdl with
  | None ->
    let ty = Bir_named_struct_type (name, ref [||]) in
    bir_type_decls := Array.append !bir_type_decls [| (name, ty) |];
    ty
  | Some ty -> ty
...
let lookup_function name { bir_function_decls } =
  Utils.find !bir_function_decls (fun (fname, func) -> fname = name)

let define_global name t ((_, x) as v) { bir_global_decls } =
  let nid = next_id () in
  bir_global_decls := Array.append !bir_global_decls [| (name, nid, v) |];
  (nid, x)
...
let declare_function name ftype ({ bir_function_decls } as mdl) =
  ...
  | None -> begin
      ...
      | Bir_function_type (_, tin) ->
        ...
        bir_function_decls :=
          Array.append !bir_function_decls [| (name, func) |];
        func
...
let append_block ctx name func =
  ...
  | (_, Bir_function (blks, _, _, _)) ->
    let tail = (name ^ (string_of_int (next_id ())), func, ref [||]) in
...
let insertion_block { bir_current_block } =
  !bir_current_block
...
let position_at_end blk { bir_current_block } =
  bir_current_block := blk

let build_struct_gep base i f { bir_current_block } =
  let inst = Bir_gep (base,
                      [| (next_id (), Bir_const_integer 0);
                         (next_id (), Bir_const_integer i) |],
                      f) in
  append_inst (next_id (), inst) !bir_current_block

let build_gep base indices name { bir_current_block } =
  let inst = Bir_gep (base, Array.of_list indices, name) in
  append_inst (next_id (), inst) !bir_current_block

let build_load v name { bir_current_block } =
  let inst = Bir_load (v, name) in
  append_inst (next_id (), inst) !bir_current_block

let build_store v p { bir_current_block } =
  let inst = Bir_store (v, p) in
  append_inst (next_id (), inst) !bir_current_block

let build_add lhs rhs { bir_current_block } =
  let inst = Bir_add (lhs, rhs) in
  append_inst (next_id (), inst) !bir_current_block

let build_mul lhs rhs { bir_current_block } =
  let inst = Bir_mul (lhs, rhs) in
  append_inst (next_id (), inst) !bir_current_block

let build_sub lhs rhs { bir_current_block } =
  let inst = Bir_sub (lhs, rhs) in
  append_inst (next_id (), inst) !bir_current_block

let build_icmp op lhs rhs { bir_current_block } =
  let inst = Bir_icmp (op, lhs, rhs) in
  append_inst (next_id (), inst) !bir_current_block

let build_call f args name { bir_current_block } =
  let inst = Bir_call (f, args) in
  append_inst (next_id (), inst) !bir_current_block

let build_extractvalue b i name { bir_current_block } =
  let inst = Bir_extractvalue (b, i, name) in
  append_inst (next_id (), inst) !bir_current_block

let build_insertvalue b v i name { bir_current_block } =
  let inst = Bir_insertvalue (b, v, i, name) in
  append_inst (next_id (), inst) !bir_current_block

let build_array_alloca t n name { bir_current_block } =
  let inst = Bir_array_alloca (t, n, name) in
  append_inst (next_id (), inst) !bir_current_block

let build_alloca t name { bir_current_block } =
  let inst = Bir_alloca (t, name) in
  append_inst (next_id (), inst) !bir_current_block

let build_ret v { bir_current_block } =
  let inst = Bir_ret v in
  append_inst (next_id (), inst) !bir_current_block

let build_global_stringptr s name { bir_current_block } =
  let inst = Bir_global_stringptr (s, name) in
  append_inst (next_id (), inst) !bir_current_block

let build_br b { bir_current_block } =
  let inst = Bir_br b in
  append_inst (next_id (), inst) !bir_current_block

let build_cond_br p then_bb else_bb { bir_current_block } =
...
let rec string_of_inst (_, x) =
  ...
  | Bir_nil  -> "nil"
  | Bir_gep _  -> "gep"
  | Bir_const_integer i -> "int[" ^ (string_of_int i) ^ "]"
