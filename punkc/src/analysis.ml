...
let rec check_mut_expr env e =
  ...
  | Efunc (vmcl, c, s) ->
    ...
    Efunc (vmcl, c, check_mut_stmt env s)
  | _ -> e

and check_mut_stmt env s =
  match s with
  | Sexpr e -> Sexpr (check_mut_expr env e)
  | Sblk (hd::next) ->
    begin match hd with
      | Sdecl((id, _), Mutable, _, e) ->
  ...
  | Sblk [] -> s
  | Sret e -> Sret (check_mut_expr env e)
  | Sif (e, s0, s1) ->
    Sif (check_mut_expr env e, check_mut_stmt env s0, check_mut_stmt env s1)
  | Swhile (e, s') ->
    Swhile (check_mut_expr env e, check_mut_stmt env s')
  | Sdecl (v, m, x, e) ->
    Sdecl (v, m, x, check_mut_expr env e)
  | Sasgn (e0, e1) ->
    if is_mutable env e0 then
      Sasgn (e0, check_mut_expr env e1)
    else
